...
Note : le risque relatif est plus facilement interoprétrable que l’*odds ratio* et si $p$ est petit alors $\frac{p}{1-p} \simeq p$ et l’*odds ratio* est proche du risque relatif.


**Interoprétation**, $OR$ et $RR$ donnent la même indication sur la relation entre $Y$ et $X$ :
...
d = data.frame(do.call( rbind, list(
...
set.seed(1)
d = d[sample(1:nrow(d)),]
colnames(d) = c("cancer", "tabac", "alcool")
rownames(d) = NULL
```
...
3. Réaliser le test de rapport ce vraisemblance à partir des deux valeurs précédemment calculée
...
6. Réaliser le test de rapport ce vraisemblance à partir des deux valeurs précédemment calculée
